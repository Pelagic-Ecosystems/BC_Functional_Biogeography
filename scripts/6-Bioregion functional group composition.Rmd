...
This file calculates the functional group composition of the bioregions. The relative abundance and biomass of functional groups in trait space are also visualized. The composition by functional groups are calculated in chunks 2 and 3 using the species level community data but there results of the calculations are stored in the bioregeions_fg_*.RData file. Bioregions are compared according to the absolute and relative abundance and biomass of functional groups.
...
# The calculated functional group composition per bioregion
load(here("data/bioregions_fg_abundance_biomass_20250217.RData"))
...
This calculates the average abundance and biomass from the community data. Outputs are already exported to a data file loaded in the next chunk.
```{r, eval=FALSE}
...
save(zoop.bc.fun, sp.regions,
     file = here("data/bioregions_fg_abundance_biomass_20250217.RData"))
```


## FigS4 Species density in trait space
...
cowplot::plot_grid(hplot.abund, hplot.biom, labels = "auto")

ggsave(filename = here("figures/fig_S4_traits_space_ab_PC12.png"),
    width = 10, height= 12, units = "in", dpi = 600, bg = "white")
```


# 4. Table S3 Bioregion composition comparisons
```{r}
...
#           file = here("tables/table_S3_bioregion_fg_means_to_annotate_20240704.csv"), row.names = FALSE)

```


# 5. Plot relative abundance and biomass
...
ggsave(filename = here("figures/treemap_biomass_offshore.png"),
...
ggsave(filename = here("figures/treemap_biomass_deepshelf.png"),
width = 4, height= 4, units = "in", dpi = 600, bg = "white")
...
ggsave(filename = here("figures/treemap_biomass_nearshore.png"),
width = 4, height= 4, units = "in", dpi = 600, bg = "white")
...
ggsave(filename = here("figures/treemap_biomass_deepfjord.png"),
width = 4, height= 4, units = "in", dpi = 600, bg = "white")
```


# 7. Composition significance tests
