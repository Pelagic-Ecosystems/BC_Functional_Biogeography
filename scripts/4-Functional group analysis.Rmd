...
trait.cor.list <- data.frame(
  ...
  label = c("Dry weight","Water content", 
                ...
                "Ws. ammonium excretion rate", "Ws. phosphate excretion rate",
                ...
                "Ind. ammonium excretion rate", "Ind. phosphate excretion rate",
...
# 5. Fig2 Dendrogram and cluster names
...
# 6. FigS2 Distirbution of each species in a functional group along each trait axis.
## Numeric traits 
```{r, eval=TRUE}
...
ggsave(file = here("figures/fig_s2_fungrps_values.png"),
       height = 14, width = 12, dpi = 300)
```

# 7. Fig3 Trait space plots
...
## Fig 3 PCoA 1 and 2
```{r}
# Correctly label axes as PCoA
axisname <- colnames(pcoa.vec[,c(2:6)])
axisname_label <- str_replace(axisname, "PC", "PCoA ")
...
g1 <- ggplot(pcoa.vec, aes_string(x = axisname[axestoplot[1]], 
  ...
  labs(x = axisname_label[axestoplot[1]], 
       y = axisname_label[axestoplot[2]]) +
...
ggsave(filename = here("figures/fig_3_pcoa12_withlabels.png"),
    width = 15, height= 10, units = "in", dpi = 600, bg = "white")
...
# Plot trait space
g3 <- ggplot(pcoa.vec.2, aes_string(x = axisname[axestoplot[1]], 
                            y = axisname[axestoplot[2]], color = "majorgroup")) + 
  labs(x = axisname_label[axestoplot[1]], 
       y = axisname_label[axestoplot[2]]) +
  geom_vline(xintercept = 0, colour='darkgray') + 
  ...
  geom_point(data = U1, color = "grey80", shape = 8) +
...
traits.num.2 <- distinct(pcoa.vec, Species) %>% 
  ...
  column_to_rownames("Species")
# Adjust column names
trait.labels <- c("%water", "%C", "%N","%P", 
                  "C:N", "N:P", "C:P", 
                  "body length", "dry weight", 
                  "C weight",
                  "N weight", "P weight",
                  "ws. respiration", "ws. clearance",
                  "ws. N excretion", "ws. P excretion",
                  "ind. respiration", "ind. clearance",
                  "ind. N excretion", "ind. P excretion")
...
maxpos <- 1/max(abs(U1))
pcoa.vec.3 <- pcoa.vec 
pcoa.vec.3[axisname[axestoplot]] <- pcoa.vec.3[axisname[axestoplot]]* maxpos*2
# Plot trait space
g4 <- ggplot(pcoa.vec.3, aes_string(x = axisname[axestoplot[1]], 
                            y = axisname[axestoplot[2]], color = "majorgroup")) + 
  labs(x = axisname_label[axestoplot[1]],
       y = axisname_label[axestoplot[2]]) +
  geom_vline(xintercept = 0, colour='darkgray') + 
...
# Plot trait space
g5 <- ggplot(pcoa.vec, aes_string(x = axisname[axestoplot[1]], 
                            y = axisname[axestoplot[2]], color = "fungrp")) + 
  labs(x = axisname_label[axestoplot[1]], 
       y = axisname_label[axestoplot[2]]) +
  geom_polygon(data = hulls.global, aes(x=xdata,y=ydata,fill=level),
...
# Plot trait space
g6 <- ggplot(pcoa.vec, aes_string(x = axisname[axestoplot[1]], 
                            y = axisname[axestoplot[2]], color = "fungrp")) + 
  labs(x = axisname_label[axestoplot[1]], 
       y = axisname_label[axestoplot[2]]) +
  geom_polygon(data = hulls.global, aes(x=xdata,y=ydata,fill=level),
...
g8 <- ggplot(pcoa.vec.2, aes_string(x = axisname[axestoplot[1]], 
  ...
  geom_segment(data = U3, aes(x = rep(0,nrow(U3)),y =rep(0,nrow(U3)),
                              xend = U3[,1],yend = U3[,2]),
               lineend = "round", size = 0.5, colour = "grey80") +
  geom_point(data = U3, color = "grey80", shape = 8)+
...
U0 <- traits.pcoa.cor.sig %>% 
  ...
  filter(r >= 0.33) %>% 
  distinct(trait)
U4 <- traits.pcoa.cor[,c(1,axestoplot+1)] %>% 
  filter(trait %in% U0$trait) %>%
  column_to_rownames("trait")
maxpos <- 1/max(abs(U4))
pcoa.vec.2 <- pcoa.vec 
...
g9 <- ggplot(pcoa.vec.2, aes_string(x = axisname[axestoplot[1]], 
  ...
  theme( aspect.ratio = 1, legend.position="none", 
        ...
        axis.title = element_text(size = 12),
        axis.text = element_text(size = 12)) + 
  geom_segment(data = U4, aes(x = rep(0,nrow(U4)),y =rep(0,nrow(U4)),
                             xend = U4[,1],yend = U4[,2]),
               lineend = "round", size = 0.5, colour = "grey80") +
  geom_point(data = U4, color = "grey80", shape = 8)+
  geom_text_repel(data = U4, aes(label=rownames(U4),), 
                  color="black", size = 4) +
  xlim(min(U4[,1])-0.05, max(U4[,1])+0.05) +
  ylim(min(U4[,2])-0.1, max(U4[,2])+0.05)
...
U0 <- traits.pcoa.cor.sig %>% 
  ...
  filter(r >= 0.33) %>% 
  distinct(trait)
U5 <- traits.pcoa.cor[,c(1,axestoplot+1)] %>% 
  filter(trait %in% U0$trait) %>%
  column_to_rownames("trait")
maxpos <- 1/max(abs(U5))
pcoa.vec.2 <- pcoa.vec 
pcoa.vec.2[axisname[axestoplot]] <- pcoa.vec.2[axisname[axestoplot]]* maxpos *2
# Plot trait space
g10 <- ggplot(pcoa.vec.2, aes_string(x = axisname[axestoplot[1]], 
                            y = axisname[axestoplot[2]], color = "majorgroup")) + 
  labs(x = axisname_label[axestoplot[1]],
         y = axisname_label[axestoplot[2]]) +
  geom_vline(xintercept = 0, colour='darkgray') + 
  ...
  theme(aspect.ratio = 1, legend.position="none", 
        ...
        axis.title = element_text(size = 12),
        axis.text = element_text(size = 12)) + 
  geom_segment(data = U5, aes(x = rep(0,nrow(U5)),y =rep(0,nrow(U5)),
                              xend = U5[,1],yend = U5[,2]),
               lineend = "round", size = 0.5, colour = "grey80") +
  geom_point(data = U5, color = "grey80", shape = 8)+
  geom_text_repel(data = U5, aes(label=rownames(U5),), 
                  color="black", size = 4) +
  xlim(min(U5[,1])-0.05, max(U5[,1])+0.05) +
  ylim(min(U5[,2])-0.05, max(U5[,2])+0.05)
...
# ggsave(filename = here("figures/new_figs/fig_3_supp_alternative.png"),
#     width = 10, height= 15, units = "in", dpi = 600, bg = "white")
```
